...
def main():
    ...
    background_df = create_random_background(
        num_iter = 500,
        expr_object = harmonization.expression_df.iloc[start_sample_num:end_sample_num],
        regress = regress,
        ssgsea = True,
    ...
    if regress:
        background_df.to_parquet(
            os.path.join(out_dir, f'synapse_random_background500iter_{start_sample_num}-{end_sample_num}cells_noregress_useCC{use_cell_cycle_genes}.parquet')
            )
        print(f"wrote to {os.path.join(out_dir, f'synapse_random_background500iter_{start_sample_num}-{end_sample_num}cells_noregress_useCC{use_cell_cycle_genes}.parquet')}")
    else:
        background_df.to_parquet(
            os.path.join(out_dir, f'synapse_random_background500iter_{start_sample_num}-{end_sample_num}cells_noregress_useCC{use_cell_cycle_genes}.parquet')
            )
